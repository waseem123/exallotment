...
def getClassOne():
    connection = connectivity()
    if (connection is not None):
        sql = "SELECT `emp_id`, `emp_name` FROM `tbl_employee` WHERE emp_category = 1 AND emp_status = 0"
        cursor = connection.cursor()
        cursor.execute(sql)
...
def createallotment():
    ...
    if (connection is not None):
        ...
        print(result)
        cursor.close()
    return render_template("createallotment.html", exam=exam,
                           examcenter=examcenter, strength=result[0][4], classone=getClassOne(), srclerk=getSrClerk(),
                           clerk=getClerk(),
                           peon=getPeon())


@app.route("/save-kendrapramukh", methods=["GET", "POST"])
def saveKendrapramukh():
    inputcenterhead = request.form['inputcenterhead']
    inputasstcenterhead = request.form['inputasstcenterhead']
    centerid = request.form['inputExamcenter']
    exam = request.form['inputExam']
    # designation = request.form['inputDesignation']
    connection = connectivity()
    if (connection is not None):
        sql1 = "INSERT INTO `tbl_allotment`(`exam_center_id`, `emp_id`, `appointed_designation`,exam_id) VALUES ({},{},'केंद्रप्रमुख','{}')"
        sql2 = "INSERT INTO `tbl_allotment`(`exam_center_id`, `emp_id`, `appointed_designation`,exam_id) VALUES ({},{},'केंद्रप्रमुखाचे सहाय्यक','{}')"

        cursor = connection.cursor()
        cursor.execute(sql1.format(centerid, inputcenterhead, exam))
        connection.commit()

        cursor = connection.cursor()
        cursor.execute(sql2.format(centerid, inputasstcenterhead, exam))
        connection.commit()

        cursor.close()
    return {"success":True,"message":"Saved Succesfully"}
